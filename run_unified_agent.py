    ...
    try:
        ...
        CALENDAR_CLIENT_CONFIG = {
            "web": {
                ...
                "redirect_uris": [f"{os.getenv('BACKEND_URL', 'https://6064e54e2e4d.ngrok-free.app')}/schedule-agent/auth/google/callback"]
            }
        }
...
async def calendar_callback(code: str, state: str):
    ...
    try:
        ...
        # TODO: Store the calendar tokens securely (in your database or encrypted storage)
        # For now, we'll log them so you can add to .env
        print(f"📝 GOOGLE_CALENDAR_TOKEN={creds.token}")
        print(f"📝 GOOGLE_CALENDAR_REFRESH_TOKEN={creds.refresh_token}")
        
        # Redirect to your React Native app using the existing oauth-success format
        redirect_url = f"myapp://oauth-success?type=calendar&user_id={user_id}&consent_token_read=calendar_temp_token&consent_token_write=calendar_temp_token&next=gmail"
        print(f"🚀 Redirecting to app for Gmail setup: {redirect_url}")
